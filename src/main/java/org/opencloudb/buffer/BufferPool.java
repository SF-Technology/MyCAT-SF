...
public final class BufferPool {
    ...
    public BufferPool(long bufferSize, int chunkSize, int threadLocalPercent) {
        ...
        this.memoryUsage = new ConcurrentHashMap<>();
    }
    ...
    public ByteBuffer allocate() {
        ...
        if (node != null && node instanceof DirectBuffer) {
    ...
    public void recycle(ByteBuffer buffer) {
        ...
        if (memoryUsage.containsKey(threadId)) {
            ...
            long size = memoryUsage.get(threadId) - buffer.capacity();
            memoryUsage.put(threadId, size);

            if (memoryUsage.get(threadId) <=0 ) {
                memoryUsage.remove(threadId);
            }
        }
    ...
    public ConcurrentHashMap<Long,Long> getMemoryUsage() {
        return memoryUsage;
    }
