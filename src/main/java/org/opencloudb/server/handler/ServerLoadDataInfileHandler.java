...
public final class ServerLoadDataInfileHandler implements LoadDataInfileHandler {
    ...
    private void parseOneLine(List<SQLExpr> cols, String[] line, boolean toFile, String lineEnd) {
        RouteResultset rrs = tryDirectRoute(sql, line);
        if (rrs == null || rrs.getNodes() == null || rrs.getNodes().length == 0) {

            String insertSql = makeSimpleInsert(cols, line, tableName, true);
    ...
    private void saveDataToFile(LoadData data, String dnName) {
        if (data.getFileName() == null) {
            ...
            try {
                dnPath = new File(dnPath).getCanonicalPath();
    ...
    public void end(byte packID) {
        ...
        List<SQLExpr> cols = statement.getColumns();
        if (isHasStoreToFile) {
            ...
            try {
                ...
                while ((row = parser.parseNext()) != null) {
                    parseOneLine(cols, row, false, null);
                }
            } finally {
                parser.stopParsing();
            }
    ...
    private void parseFileByLine(String file, String encode, String split) {
        ...
        try {
            ...
            String[] row = null;
            final List<SQLExpr> cols = statement.getColumns();
            while ((row = parser.parseNext()) != null) {
                parseOneLine(cols, row, true, loadData.getLineTerminatedBy());
            }
