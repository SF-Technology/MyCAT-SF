...
public class SystemParameter {
    ...
    public final  static String [][] SYS_PARAM={
             ...
             * SQL 防火墙配置默认配置
            */
            {"enableSQLFirewall","SQL 防火墙总开关 0 关闭，不记录直接打印警告信息，1开启，2 关闭，并记录拦截信息"},
    ...
    public final static String [][] SYS_PARAM_BOOL={
             ...
             * SQL 防火墙配置默认配置
             */
            {"enableRegEx","是否启用正则表达式匹配SQL"},
            {"selectAllColumnAllow","是否允许执行SELECT * FROM T这样的语句"},
            ...
            {"deleteWhereNoneCheck","检查DELETE语句是否无where条件"},
            {"updateWhereAlayTrueCheck","检查UPDATE语句的WHERE子句是否是一个永真条件"},
            {"updateWhereNoneCheck","检查UPDATE语句是否无where条件"},
             ...
             * 其他拦截配置
            */
            {"selectIntoOutfileAllow","SELECT ... INTO OUTFILE 是否允许"},
            ...
            {"mustParameterized","是否必须参数化"},
            {"strictSyntaxCheck","是否进行严格的语法检测"},
            {"conditionOpXorAllow","查询条件中是否允许有XOR条件"},
